...
mesh_samples_subdir = "MeshSamples"
...
def load_pre_trained_latent_vectors(experiment_directory, checkpoint):
    ...
    filename = os.path.join(
        experiment_directory, latent_codes_subdir, str(checkpoint) + ".pth"
    )

    if not os.path.isfile(filename):
        raise Exception(
            "The experiment directory ({}) does not include a latent code file"
            + " for checkpoint '{}'".format(experiment_directory, str(checkpoint))
        )
...
def get_trained_mesh_filename(
    experiment_dir, epoch, dataset, class_name, instance_name, 
    corrspondence_level=None, correspondence_pts_num = 0
):
    if correspondence_pts_num != 0 and corrspondence_level is not None:
        corrspondence_suffix = f'_{correspondence_pts_num}_{corrspondence_level}_correspondence'
    else:
        corrspondence_suffix = ''
    ...
    return os.path.join(
        ...
        dataset,
        class_name,
        instance_name + corrspondence_suffix + '.ply'
    )
...
def prepare_warper(specs, experiment_directory, checkpoint):
    arch = __import__("networks." + specs["NetworkArch"], fromlist=["Warper"])

    latent_size = specs["CodeLength"]

    Warper = arch.Warper(latent_size, **specs["NetworkSpecs"]["warper_kargs"])
    ...
    pretrained_dict = saved_model_state["model_state_dict"]
    # 1. filter out unnecessary keys
    pretrained_dict = {k.replace('.warper', ''): v for k, v in pretrained_dict.items() if k.startswith('module.warper')}
    # 2. overwrite entries in the existing state dict
    # saved_model_state["model_state_dict"].update(pretrained_dict) 

    Warper.load_state_dict(pretrained_dict)

    Warper = Warper.module.cuda()

    return Warper, saved_model_epoch

def prepare_decoder(specs, experiment_directory, checkpoint):
    ...
    return decoder, saved_model_epoch

def load_optimized_test_latent_vectors(experiment_directory, checkpoint, instance_filenames):
    latent_vectors = []
    for instance_filename in instance_filenames:
        ...
        code_path = os.path.join(experiment_directory, 
                                 reconstructions_subdir, 
                                 str(checkpoint), 
                                 reconstruction_codes_subdir, 
                                 dataset_name, 
                                 class_name,
                                 '.'.join(instance_name.split('.')[:-1]) + '.pth')
    
        latent_vec = torch.load(code_path).squeeze()
        latent_vectors += [latent_vec]
